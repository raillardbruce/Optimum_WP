...
function select_postid_by_category($slug_category)
{
	...
	if (isset($_POST['reservations_cc'])) {
		...
		if ($slug_category == 'cardio-training' OR $slug_category == 'cross-training') {
			$type_reservation = 1;
		}
		if ($slug_category == 'renforcement-musculaire' OR $slug_category == 'preparation-physique') {
			$type_reservation = 2;
		}
		if ($slug_category == 'yoga') {
			$type_reservation = 3;
		}
		$type_sport = $slug_category;
...
function show_reservation_yoga_form()
{
	...
	select_postid_by_category('yoga');
...
function show_sessions_cc()
{
	...
	$sessions_cc = $wpdb->get_results("SELECT object_Id FROM $table_name_post WHERE term_taxonomy_id = 4", ARRAY_A);
	...
	foreach ($sessions_cc as $session_cc) {
		...
		foreach ($sessions_taxonomy as $session_taxonomy_01) {
			...
			if ($session_taxonomy_id == 5) {
				echo '<a href="http://bruce.devweb.cfa.nc/fitness-nc/reservation-cross-training/" class="btn btn-primary">Inscription</a>';
			}
			if ($session_taxonomy_id == 3) {
				echo '<a href="http://bruce.devweb.cfa.nc/fitness-nc/reservation-cardio-training/" class="btn btn-primary">Inscription</a>';
			}
		}
		echo '</div></div>';
	}
...
function show_sessions_cp()
{
	...
	$sessions_cc = $wpdb->get_results("SELECT object_Id FROM $table_name_post WHERE term_taxonomy_id = 6", ARRAY_A);
	...
	foreach ($sessions_cc as $session_cc) {
		...
		foreach ($sessions_taxonomy as $session_taxonomy_01) {
			$session_taxonomy_id = implode(", ", $session_taxonomy_01);
			if ($session_taxonomy_id == 7) {
				echo '<a href="http://bruce.devweb.cfa.nc/fitness-nc/formulaire-de-reservation-renforcement-musculaire/" class="btn btn-primary">Inscription</a>';
			}
			if ($session_taxonomy_id == 8) {
				echo '<a href="http://bruce.devweb.cfa.nc/fitness-nc/formulaire-de-reservation-preparation-physique/" class="btn btn-primary">Inscription</a>';
			}
		}
		echo '</div></div>';
	}
...
function show_sessions_yoga()
{
	...
	$sessions_cc = $wpdb->get_results("SELECT object_Id FROM $table_name_post WHERE term_taxonomy_id = 9", ARRAY_A);
	...
	foreach ($sessions_cc as $session_cc) {
		...
		echo '<h5 class="card-title">' . $session_cc_title . '</h5>';
		echo '<p class="card-text">' . $session_cc_content . '</p>';
		echo '<a href="http://bruce.devweb.cfa.nc/fitness-nc/formulaire-de-reservation-yoga/" class="btn btn-primary">Inscription</a>';
		echo '</div></div>';
	}
